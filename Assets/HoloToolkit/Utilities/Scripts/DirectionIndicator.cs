...
namespace HoloToolkit.Unity
{
    ...
    public class DirectionIndicator : MonoBehaviour
    {
        ...
        private bool isDirectionIndicatorVisiblePrev;

        private event System.Action gotVisible;
        public event System.Action GotVisible
        {
            add { if (gotVisible == null || !gotVisible.GetInvocationList().Contains(value)) gotVisible += value; }
            remove { gotVisible -= value; }
        }

        private event System.Action gotInvisible;
        public event System.Action GotInvisible
        {
            add { if (gotInvisible == null || !gotInvisible.GetInvocationList().Contains(value)) gotInvisible += value; }
            remove { gotInvisible -= value; }
        }
        ...
        public void Update()
        {
            ...
            isDirectionIndicatorVisiblePrev = isDirectionIndicatorVisible;
            ...
            {
                if (!isDirectionIndicatorVisiblePrev)
                {
                    //got visible
                    gotVisible?.Invoke();
                }
